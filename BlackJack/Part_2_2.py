...
#this creats out Q-value look up table
...
#setup a dictionary of state-actions to record how many times we/ve expefrienced
#a given state-action pair. we use this to re-calculate reward averages
...
#this calculates the reward of the game, either +1 for winning, 0 for draw, -1 for losing
#we determine this by simply subtracting the game status value from 3
def calcReward(outcome):
    return 3-outcome

#this re-calculates the average rewards for our Q-value look-up table
...
#converts a game state of the form ((play total, ace), (dealer total, ace), status)
...
for i in range(epochs):
    ...
    #we do this because whenever a player's total is less than 11 you always hit no matter what
    #so we don't want to waste compute cycles on that subset of the state state_space
    ...
    while(state[2] == 1): #while in current episode
        ...
        sa = ((rl_state, action))
        returns[sa] = 0 #add a-v pair to retuns list, default value to 0
    ...
    # after an episode is comple, assigne rewards to all the state-actions that took place in the episode
    for key in returns:
